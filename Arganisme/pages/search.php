			...
			<div class="row">
				<?php
				    ...
				    while (($row = $statement->fetch())){
			    		if ($cc == $row['name'] || $cc == strtolower($row['name']) || $cc == strtoupper($row['name']) ||str_contains($row['name'], $cc)) {
			    			echo '
							<div class="product col">
								<a href="product.php?name='.$row['name'].'">
									<div class="img2" id="'.$row['id'].'" style="background-image: url(../images/'.$row['image'].');">
									</div>
								</a>
								<p>'.$row['name'].'</p>
							</div>
