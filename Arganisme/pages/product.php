...
<body>
	...
	<div id="content" class="container" align="center">
		<section class="sec" id="productPage">
			<div class="row">
				<?php 
					if (!empty($_GET['name'])) {
						$ide = $_GET['name'];
					...
					$statement = $connection->prepare('SELECT * FROM products WHERE name = :ide');
					$statement->bindParam(':ide', $ide, PDO::PARAM_STR);
					$statement->execute();
					$result = $statement->fetch();
					if ($result) {
						$cat = $result['category'];
					}
					$statement2 = $connection->prepare('SELECT products.id, products.name, products.description , products.image, categories.name AS category FROM products LEFT JOIN  categories ON products.category = categories.id WHERE categories.id = :cat');
					$statement2->bindParam(':cat', $cat, PDO::PARAM_INT);
					$statement2->execute();
					$products = $statement2->fetch();
					$repos = "'#productOrderFormular'";
					if ($result) {
						echo '
						<div class="offProduct col">
							<a >
								<div class="img3" id="i1" style="background-image: url(../images/'.$result['image'].');">
								</div>
							</a>
						</div>
						<div class="textArea col">
							<h2 class="textAreaItem" id="productTitle">'.$result['name'].'</h2>
							<h5 class="textAreaItem" id="productCategorie">'.$products['category'].'</h5>
							<p class="textAreaItem" id="productDescription">'.$result['description'].'</p>
							<button align="center" onclick="location.href='.$repos.';" class="productOrder">Order Now</button>
						</div>';
					}
				?>
					...
					foreach ($results as $products) {
						echo '
							<div class="product col">
								<a href="product.php?name='.$products['name'].'">
			...
			<div class="Formular" id="productOrderFormular" align="left">
	            <form id="contact-form" method="post" action="orderHandler.php" role="form">
	                <div class="row">
	                    <div class="col-lg-6">
	                    ...
	                    <div class="col-lg-6">
	                        ...
	                        <input id="productProd" type="text" name="product" class="form-control" placeholder="Your product" value="">
	                        <p class="comments"></p>
	                    </div>
	                    <div>
